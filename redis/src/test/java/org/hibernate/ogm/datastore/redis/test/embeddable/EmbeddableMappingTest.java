	...
	public void testEmbeddableCollection() throws Exception {
		...
		story.setGoodBranch(
				new StoryBranch(
						...
						new Ending( "village ending - everybody is happy", 1 )
				)
		);
		story.setEvilBranch( new StoryBranch( "you kill the villagers" ) );
		story.setChaoticBranches(
				Arrays.asList(
						new OptionalStoryBranch( "search the evil [artifact]", "you punish the bandits", null ),
						new OptionalStoryBranch(
								"assassinate the leader of the party", null, new Ending(
								"you become a demon",
								10
						)
						)
