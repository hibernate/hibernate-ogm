...
public abstract class RecognizerBasedParameterMetadataBuilder implements ParameterMetadataBuilder {
	...
	public ParameterMetadataImpl buildParameterMetadata(String nativeQuery) {
		...
		// After a quick inspeciton it seems that only with Neo4j would be easy to make it work
		// and even in that scenario I'm not sure we should care.
		// I think it makes sense to keep track of the JPA compliance problem in a separate issues.
