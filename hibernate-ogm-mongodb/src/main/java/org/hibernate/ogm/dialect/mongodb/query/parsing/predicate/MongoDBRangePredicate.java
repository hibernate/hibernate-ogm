...
public class MongoDBRangePredicate extends RangePredicate<DBObject> implements NegatablePredicate<DBObject> {
	...
	public DBObject getQuery() {
		return new BasicDBObject(
				...
				Arrays.<DBObject>asList(
						new BasicDBObject( "$gte", new BasicDBObject( propertyName, lower ) ),
						new BasicDBObject( "$lte", new BasicDBObject( propertyName, upper ) )
				)
		);
	}
	...
	public DBObject getNegatedQuery() {
		return new BasicDBObject(
				"$or",
				Arrays.<DBObject>asList(
						new BasicDBObject( "$lt", new BasicDBObject( propertyName, lower ) ),
						new BasicDBObject( "$gt", new BasicDBObject( propertyName, upper ) )
				)
		);
	}
}
