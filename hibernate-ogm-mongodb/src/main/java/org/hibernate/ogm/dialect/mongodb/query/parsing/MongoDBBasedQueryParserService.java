...
/**
 * {@link QueryParserService} implementation which creates MongoDB queries in form of {@link DBObject}s.
 *
 * @author Gunnar Morling
 */
public class MongoDBBasedQueryParserService implements QueryParserService {
	...
	public Query getParsedQueryExecutor(OgmSession session, String queryString, Map<String, Object> namedParameters) {
		...
		MongoDBProcessingChain processingChain = createProcessingChain( session, namedParameters );

		MongoDBQueryParsingResult result = queryParser.parseQuery( queryString, processingChain );
		DBObject projections = result.getProjection();
		log.createdMongoDBQuery( queryString, result.getQuery(), projections );

		return new MongoDBQueryImpl( result.getEntityType(), result.getQuery(), projections, session );
	}
