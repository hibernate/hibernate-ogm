...
The Neo4j dialects don't not require Hibernate Search to run JPQL or HQL queries.
...
map and the embedded values will be store in a node with only the `EMBEDDED` label.
We don't think this is a natural mapping and expect it to change in the upcoming relaseases. 
