...
* Use the stored procedure mapping the result as managed entities
...
=== Using stored procedures of your NoSQL

Often you want the raw power of the underlying NoSQL query engine.
Even if that costs you portability.

Hibernate OGM addresses that problem by letting you express stored procedures (e.g. in Server-side JavaScript for MongoDB )
and map the result of these queries as mapped entities or return primitive result.
...
@NamedStoredProcedureQueries({
		@NamedStoredProcedureQuery(name = "find_cars_by_title", procedureName = "resultSetResultProc", parameters = {
...
// a primitive result call
StoredProcedureQuery storedProcedureQuery = em.createStoredProcedureQuery( "simpleResultProc" );
...
// a result set call
StoredProcedureQuery storedProcedureQuery = em.createStoredProcedureQuery( "resultSetResultProc", Car.class );
storedProcedureQuery.registerStoredProcedureParameter( 0, Void.class, ParameterMode.REF_CURSOR );
storedProcedureQuery.registerStoredProcedureParameter( 1, String.class, ParameterMode.IN );
storedProcedureQuery.setParameter( 1, "title" );
List<Car> cars = storedProcedureQuery.getResultList();


// named stored procedure query
StoredProcedureQuery storedProcedureQuery = em.createNamedStoredProcedureQuery( "find_cars_by_title" );
storedProcedureQuery.setParameter( 1, "title" );
List<Car> cars = storedProcedureQuery.getResultList();
...
Check out each individual datastore chapter for more info
on the specifics of the native query language mapping.
In particular  <<ogm-mongodb-stored-proc-native,MongoDB>>.

[WARNING]
====
'OUT' and 'IN_OUT' parameters of stored procedures are not supported yet.
Main reason of it is supported data storages are supports 'IN' parameters only.
====
