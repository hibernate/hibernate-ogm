...
Boolean flag to enable Redis Cluster mode.
...
Curly braces within keys are used to denote http://redis.io/topics/cluster-spec#keys-hash-tags[hash tags] to group keys within one slot.
...
class should be stored in separate association documents.
...
You can specify a different TTL on associations if they are stored as separate documents. In the case of `visitors`
which is stored in the entity, the TTL of the association will be the one of the entity.
...
[
	...
	{"value":{\"number\":\"2000\",\"project\":\"OGM\"},\"issues_KEY\":\"issue2\"}"
	{"value":{\"number\":\"1253\",\"project\":\"HSEARCH\"},\"issues_KEY\":\"issue1\"}
]
----
====
...
[NOTE]
====
The value stored in Redis for the sequence is not the one the dialect will return as next number
...
Using Hibernate OGM with Redis Cluster is a matter of enabling Redis Cluster mode (see <<ogm-redis-configuration>>).
Data stored with Hibernate OGM Redis is distributed across Redis Cluster nodes
according to the http://redis.io/topics/cluster-spec#keys-distribution-model[Redis key distribution model]
based on the hash slot of the key of each entity.
