...
Alternatively you can lookup datastore client. See <<integration-with-wildfly-nosql>>.
...
hibernate.ogm.mongodb.gridfs.bucket.name::
Defines common bucket name for GridFs using.
...
[source, JSON]
----
  { "content_id" : ObjectId("567d9b60f62028750f25ef88") }
----
The type is used for storing binary content to GridFS. See details in <<mongodb-gridfs>>
...
Person  collection as JSON in MongoDB
...
[[mongodb-gridfs]]
==== GridFS Using
GridFS is specification for working (store and load) with large files (more 16M). Details about
GridFS you can read in https://docs.mongodb.com/manual/core/gridfs/[GridFS manual].

.Mapping for an entity with using storing content in GridFS
====
[source, JAVA]
----
@Entity
...
[source, JSON]
----
// Stored in the Collection "Photo"
{
    "_id" : ObjectId("367d0b60a63123751425ef91"),
    "title": "I am at Black Sea in Sochi",
    "content_id" : ObjectId("567d9b60f62028750f25ef88")
}
----
...
Entity with GridFs is configurable.
GridFS is manageable by:

* annotation `@GridFSBucket` have a parameter with bucket name. if bucket name not specified, Hibernate OGM
uses default bucket name of MongoDB or value from property `hibernate.ogm.mongodb.gridfs.bucket.name`.
* annotation `@BinaryStorage` specify type of binary storage. Version 5.2.0 supports 'GRID_FS' only.
