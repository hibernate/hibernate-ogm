...
If you want to override one column name but still keep the nested structure, don't forget the dots.
...
public DeliveryProvider {
    @Column(name="delivery.provider")
    String name;
    @Column(name="delivery.delays")
...
While you can use JP-QL for simple queries, you might hit a limitation of our conversion.
