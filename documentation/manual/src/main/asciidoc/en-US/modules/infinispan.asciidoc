...
+hibernate.ogm.infinispan.persistence_strategy+::
The strategy for persisting data in Infinispan.
The following two strategies exist (values of the +org.hibernate.ogm.datastore.infinispan.options.PersistenceStrategy+ enum):
+CACHE_PER_KIND+
