...
The +onFailedGridDialectOperation()+ method lets you decide whether to continue with the application of the changes of the current flush cycle or abort it. If +ABORT+ is returned, the causing exception will be re-thrown, eventually causing the current transaction to be rolled back. If +CONTINUE+ is returned, that exception will be ignored, causing the current transaction to continue.
