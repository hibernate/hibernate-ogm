...
Additional datastore providers are provided through the Hibernate OGM Contributions project.
These providers are incubating state and/or may be led by members from the Hibernate OGM community.
Incubating providers should be used with great care,
as their approach for persisting data in the corresponding NoSQL store may change in future revisions.
Currently there is a provider for the CouchDB document data store in the Hibernate OGM Contributions project.
Refer to its documentation to learn more about the details.
