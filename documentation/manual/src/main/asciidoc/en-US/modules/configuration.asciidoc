...
[WARNING]
====
The EAP 6 support in Hibernate OGM is  *experimental*.
The contents of existing modules of the application server
(_javax.persistence.api_, _javax.ws.rs.api:main_ and _org.javassist:main_) will be replaced with the versions required by Hibernate OGM
when extracting the module ZIP into the application server directory.
It is therefore recommended to use this module ZIP only for testing purposes.
Efforts are underway to avoid this in future revisions (see https://hibernate.atlassian.net/browse/OGM-499[OGM-499]).
====

You can download the pre-packaged module ZIP from:

* https://downloads.sourceforge.net/project/hibernate/hibernate-ogm/{hibernate-ogm-version}/hibernate-ogm-modules-eap6-{hibernate-ogm-version}-experimental.zip[Sourceforge]
* https://repository.jboss.org/nexus/service/local/artifact/maven/redirect?r=central&g=org.hibernate.ogm&a=hibernate-ogm-modules-eap6-experimental&v={hibernate-ogm-version}&e=zip[JBoss's Maven repository]
