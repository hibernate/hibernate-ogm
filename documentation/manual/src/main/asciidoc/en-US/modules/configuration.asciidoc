...
The Hibernate Search documentation has a similar section http://docs.jboss.org/hibernate/search/{hibernate-search-module-slot}/reference/en-US/html/search-configuration.html#_update_and_activate_latest_hibernate_search_version_in_wildfly[Update and activate latest Hibernate Search version in WildFly].

In practice your application will need to use both Hibernate OGM and Hibernate Search, so your MANIFEST.MF will look like:

.Example MANIFEST.MF for an application using Hibernate OGM for MongoDB and also Hibernate Search
====
[source]
[subs="verbatim,attributes"]
----
org.hibernate:ogm services, org.hibernate.ogm.mongodb services, org.hibernate.search.orm:{hibernate-search-module-slot} services
----
====
