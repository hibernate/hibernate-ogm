...
When using WildFly several of the technologies it includes are automatically enabled, for example Hibernate ORM is made avaialble to your applications if and when you
have a `persistence.xml` defining a persistence unit which is using the Hibernate persistence provider, or not specifying any provider as Hibernate is the default one.
