...
public abstract class Neo4jJpaTestCase extends JpaTestCase {
	...
	protected void executeCypherQuery(String query, Map<String, Object> parameters) throws Exception {
		...
		if ( datastoreProvider instanceof Neo4jDatastoreProvider ) {
			...
			StatementsResponse statementsResponse = provider.getDataStore().executeQueriesInNewTransaction( statements );
	...
	private Long executeRemoteCount(String queryString) throws Exception {
		...
		StatementsResponse response = datastoreProvider.getDataStore().executeQueriesInNewTransaction( statements );
