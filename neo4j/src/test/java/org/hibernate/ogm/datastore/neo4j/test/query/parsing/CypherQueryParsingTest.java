...
public class CypherQueryParsingTest extends OgmTestCase {
	...
	public void shouldCreateNotInQuery() {
		assertQuery(
				...
				"MATCH (e:IndexedEntity) WHERE NOT HAS(e.title) OR  NONE( _x_ IN [\"foo\", \"bar\", \"same\"] WHERE e.title = _x_) RETURN e" );
	}
	...
	public void shouldCreateNotLikeQuery() {
		assertQuery(
				...
				"MATCH (e:IndexedEntity) WHERE NOT HAS(e.title) OR NOT(e.title=~\"^\\\\QAli\\\\E.\\\\Qe\\\\E.*$\") RETURN e" );
	}
	...
	public void shouldCreateIsNullQuery() {
		assertQuery(
				...
				"MATCH (e:IndexedEntity) WHERE NOT HAS(e.title) RETURN e" );
	}
	...
	public void shouldCreateIsNotNullQuery() {
		assertQuery(
				...
				"MATCH (e:IndexedEntity) WHERE HAS(e.title) RETURN e" );
	}
	...
	private Neo4jProcessingChain setUpProcessingChain(Map<String, Object> namedParameters) {
		...
		return new Neo4jProcessingChain( (SessionFactoryImplementor) sessions, nameResolver, namedParameters );
	}
