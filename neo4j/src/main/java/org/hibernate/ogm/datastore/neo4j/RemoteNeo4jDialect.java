...
public class RemoteNeo4jDialect extends BaseNeo4jDialect {
	...
	private Map<EntityKeyMetadata, RemoteNeo4jEntityQueries> initializeEntityQueries(SessionFactoryImplementor sessionFactoryImplementor) {
		...
		for ( EntityPersister entityPersister : entityPersisters ) {
			if (entityPersister instanceof OgmEntityPersister ) {
				...
				TupleContext tupleContext = ogmEntityPersister.getTupleContext( null );
		...
		for ( int i = 0; i < properties.length; i++ ) {
			...
			if ( containsProperty ) {
				...
				if ( !actualValue.equals( expectedValue )
						// TODO: Long values shorter than Integer.MAX_VALUE are returned as Integer
						&& !actualValue.toString().equals( expectedValue.toString() ) ) {
					return false;
				}
			}
	...
	public void insertOrUpdateTuple(EntityKey key, Tuple tuple, TupleContext tupleContext, TransactionContext transactionContext) {
		...
		applyTupleOperations( key, tuple, properties, toOneAssociations, statements, tuple.getOperations(), tupleContext, transactionContext );
	 ...
	 * @param action
	 */
	private Relationship putAssociationOperation(AssociationKey associationKey, AssociationOperation action, AssociationContext associationContext, TransactionContext transactionContext) {
		switch ( associationKey.getMetadata().getAssociationKind() ) {
			case EMBEDDED_COLLECTION:
				return createRelationshipWithEmbeddedNode( associationKey, associationContext, action, transactionContext );
			case ASSOCIATION:
				return findOrCreateRelationshipWithEntityNode( associationKey, associationContext, action, transactionContext );
			default:
				throw new AssertionFailure( "Unrecognized associationKind: " + associationKey.getMetadata().getAssociationKind() );
		}
	}

	private Relationship createRelationshipWithEmbeddedNode(AssociationKey associationKey, AssociationContext associationContext, AssociationOperation action, TransactionContext transactionContext) {
	...
	private Relationship findOrCreateRelationshipWithEntityNode(AssociationKey associationKey, AssociationContext associationContext, AssociationOperation action, TransactionContext transactionContext ) {
		Tuple associationRow = action.getValue();
	...
	public Association getAssociation(AssociationKey associationKey, AssociationContext associationContext, TransactionContext transactionContext) {
		...
		Long transactionId = transactionId( transactionContext );
		Node entityNode = entityQueries.get( entityKey.getMetadata() ).findEntity( dataBase, transactionId, entityKey.getColumnValues() );
		if ( entityNode == null ) {
			return null;
		}

		Map<RowKey, Tuple> tuples = createAssociationMap( associationKey, associationContext, entityKey, transactionContext );
	...
	private void applyAssociationOperation(Association association, AssociationKey key, AssociationOperation operation, AssociationContext associationContext, TransactionContext transactionContext) {
		...
		case CLEAR:
			...
			break;
		case PUT:
			putAssociationOperation( key, operation, associationContext, transactionContext );
			break;
		case REMOVE:
			removeAssociationOperation( key, operation, associationContext, transactionContext );
			break;
		}
	}
	...
	private void removeAssociationOperation(AssociationKey associationKey, AssociationOperation action, AssociationContext associationContext, TransactionContext transactionContext) {
		Long txId = transactionId( transactionContext );
	...
	public void forEachTuple(ModelConsumer consumer, EntityKeyMetadata... entityKeyMetadatas) {
		Long txId = null;// transactionId( tupleContext );
		for ( EntityKeyMetadata entityKeyMetadata : entityKeyMetadatas ) {
			...
			while ( queryNodes.hasNext() ) {
				...
				Tuple tuple = new Tuple( new RemoteNeo4jTupleSnapshot( dataBase, txId, queries, next, entityKeyMetadata, entityPersister( entityKeyMetadata ) ) );
	...
	public ClosableIterator<Tuple> executeBackendQuery(BackendQuery<String> backendQuery, QueryParameters queryParameters, TransactionContext transactionContext) {
		...
		if ( backendQuery.getSingleEntityKeyMetadataOrNull() != null ) {
			...
			return new RemoteNodesTupleIterator( dataBase, txId, queries, response, entityKeyMetadata, entityPersister );
		}
		else {
