...
public class HttpNeo4jDialect extends BaseNeo4jDialect implements RemoteNeo4jDialect {
	...
	private Map<EntityKeyMetadata, HttpNeo4jEntityQueries> initializeEntityQueries(SessionFactoryImplementor sessionFactoryImplementor) {
		...
		for ( EntityPersister entityPersister : entityPersisters ) {
			if (entityPersister instanceof OgmEntityPersister ) {
				...
				queryMap.put( ogmEntityPersister.getEntityKeyMetadata(), new HttpNeo4jEntityQueries( ogmEntityPersister.getEntityKeyMetadata(), tupleContext ) );
			}
		}
		return queryMap;
	}
	...
	public Tuple getTuple(EntityKey key, TupleContext tupleContext) {
		...
		Map<String, Node> toOneEntities = HttpNeo4jAssociatedNodesHelper.findAssociatedNodes( client, txId, owner, key.getMetadata(), tupleContext, queries );
		...
		while ( nodes.hasNext() ) {
			...
			for ( int i = 0; i < keys.length; i++ ) {
				...
				if ( RemoteNeo4jHelper.matches( nodeProperties, keyNames, keyValues ) ) {
					...
					Map<String, Node> toOneEntities = HttpNeo4jAssociatedNodesHelper.findAssociatedNodes( client, txId, node, metadata, tupleContext, queries );
					tuples[i] = new Tuple(
							new HttpNeo4jTupleSnapshot(
									node,
									metadata,
	...
	public void forEachTuple(ModelConsumer consumer, TupleContext tupleContext, EntityKeyMetadata entityKeyMetadata) {
		...
		while ( queryNodes.hasNext() ) {
			...
			Map<String, Node> associatedEntities = HttpNeo4jAssociatedNodesHelper.findAssociatedNodes( client, txId, next, entityKeyMetadata, tupleContext, queries );
			Tuple tuple = new Tuple( new HttpNeo4jTupleSnapshot( next, entityKeyMetadata, associatedEntities, tupleContext ) );
	...
	public ClosableIterator<Tuple> executeBackendQuery(BackendQuery<String> backendQuery, QueryParameters queryParameters, TupleContext tupleContext) {
		...
		if ( backendQuery.getSingleEntityMetadataInformationOrNull() != null ) {
			...
			return new HttpNeo4jNodesTupleIterator( client, txId, queries, response, entityKeyMetadata, tupleContext, entities );
		}
		else {
