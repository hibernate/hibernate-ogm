...
/**
 * Binds option methods (in the various entity/property/etc contexts) to their respective generator implementation. Also
 * handles the creation of a context proxy when context switching methods are called (entity() / property() etc).
 *
...
public class MappingToGeneratorInvocationHandler implements InvocationHandler {
	...
	private final Method ENTITIES_FROM_GLOBAL;
	private final Method ENTITIES_FROM_ENTITY;
	private final Method ENTITIES_FROM_PROPERTY;

	private final Method PROPERTY_FROM_ENTITY;
	private final Method PROPERTY_FROM_PROPERTY;

	private final MappingContext context;

	public MappingToGeneratorInvocationHandler(MappingContext context) {
		this.ENTITIES_FROM_GLOBAL = loadEntityMethod( GlobalContext.class );
		this.ENTITIES_FROM_ENTITY = loadEntityMethod( EntityContext.class );
		this.ENTITIES_FROM_PROPERTY = loadEntityMethod( PropertyContext.class );
		this.PROPERTY_FROM_ENTITY = loadPropertyMethod( EntityContext.class );
		this.PROPERTY_FROM_PROPERTY = loadPropertyMethod( PropertyContext.class );
	...
	private Object returnProxyForCurrentProperty(Method method, Object[] args) {
		...
		context.setCurrentProperty( new PropertyKey( context.getCurrentEntity(), (String) args[0] ) );
