...
public class MongoDBDatastoreProvider extends BaseDatastoreProvider implements Startable, Stoppable, Configurable, ServiceRegistryAwareService {
	...
	private List<Runnable> startupOperations;
		...
		if ( null == mongoDb ) {
			synchronized ( this ) {
				if ( null == mongoDb ) {
					...
					if ( null != startupOperations ) {
						for ( Runnable operation : startupOperations ) {
							operation.run();
						}
						startupOperations = null;
					}
				}
			}
		}
	...
	public void addStartupOperation(Runnable operation) {
		if ( lazyGetDatabase ) {
			if ( null == startupOperations ) {
				startupOperations = new ArrayList<>();
			}
